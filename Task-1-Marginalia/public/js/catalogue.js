...
function renderGenres(genres) {
  genreBar.innerHTML = genres
    .map((g) => `<button class="chip ${g === activeGenre ? 'active' : ''}" data-genre="${g}">${g}</button>`)
    .join('');
...
async function loadBooks() {
  ...
  try {
    ...
    grid.innerHTML = `<p class="notice">${Shell.escapeHtml(err.message)}</p>`;
  }
}
...
function wireAddButtons() {
  grid.querySelectorAll('[data-add]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      Shell.toast(`Added "${btn.dataset.title}" to your basket`);
    });
  });
}
...
function cardHTML(book) {
  ...
  return `
    <article class="book-card">
      <a href="${href}">${Shell.coverHTML(book)}</a>
      <div class="book-meta">
        <span class="genre">${Shell.escapeHtml(book.genre)}</span>
        <a class="book-title" href="${href}">${Shell.escapeHtml(book.title)}</a>
        <span class="book-author">${Shell.escapeHtml(book.author)}</span>
        <div class="book-foot">
          <span class="price">${Shell.money(book.price)}</span>
          <button class="btn small" data-add="${book.id}"
                  data-title="${Shell.escapeHtml(book.title)}" ${soldOut ? 'disabled' : ''}>
