...
(async () => {
  ...
  try {
    ...
    detail.innerHTML = `<p class="notice">${Shell.escapeHtml(err.message)}</p>`;
  }
})();
...
function render(book) {
  ...
  detail.innerHTML = `
    ${Shell.coverHTML(book, 'cover-large')}
    <div class="book-detail-info">
      <span class="genre">${Shell.escapeHtml(book.genre)}</span>
      <h1>${Shell.escapeHtml(book.title)}</h1>
      <p class="byline">${Shell.escapeHtml(book.author)}, ${book.year}</p>
      <p class="price large">${Shell.money(book.price)}</p>
      <p class="stock ${soldOut ? 'out' : ''}">
        ${soldOut ? 'Out of stock' : `${book.stock} copies on the shelf`}
      </p>
      <p class="blurb">${Shell.escapeHtml(book.blurb)}</p>
      <dl class="specs">
  ...
  document.getElementById('add').addEventListener('click', () => {
    ...
    Shell.toast(`Added ${quantity()} × "${book.title}" to your basket`);
  });
}
