...
async function fillCircleOptions() {
  ...
  try {
    ...
    select.insertAdjacentHTML('beforeend', circles
      .map((c) => `<option value="${c.slug}">${Shell.escapeHtml(c.name)}</option>`)
      .join(''));
  } catch {
...
function personHTML(person) {
  return `
    <div class="person">
      ${Shell.avatarHTML(person, 'tiny')}
      <a href="/profile.html?handle=${encodeURIComponent(person.handle)}">${Shell.escapeHtml(person.displayName)}</a>
      <button class="chip-btn" data-follow="${Shell.escapeHtml(person.handle)}">Follow</button>
    </div>`;
}
